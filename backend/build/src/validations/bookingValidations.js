...
exports.createBookingValidation = [
    // User ID validation
    validation_factory_ts_1.validator.integer('userId', {
        required: true,
        min: 1,
    }),
    // Tour ID validation (optional)
    validation_factory_ts_1.validator.integer('tourId', {
        required: false,
        min: 1,
    }),
    // Hotel ID validation (optional)
    validation_factory_ts_1.validator.integer('hotelId', {
        required: false,
        min: 1,
    }),
    // Room ID validation (optional)
    validation_factory_ts_1.validator.integer('roomId', {
        required: false,
        min: 1,
    }),
    // Flight ID validation (optional)
    validation_factory_ts_1.validator.integer('flightId', {
        required: false,
        min: 1,
    }),
    // Total price validation
    validation_factory_ts_1.validator.number('totalPrice', {
        required: true,
        min: 0,
        allowDecimals: true,
    }),
    // Custom validation to ensure at least one booking type is provided
    validation_factory_ts_1.validator.custom('bookingType', (value, req) => {
    ...
    validation_factory_ts_1.validator.custom('roomHotelConsistency', (value, req) => {
...
exports.updateBookingValidation = [
    ...
    validation_factory_ts_1.validator.enum('status', ['PENDING', 'CONFIRMED', 'CANCELLED', 'COMPLETED'], {
        required: false,
    }),
    // Custom validation to ensure room is associated with hotel if both are provided
    validation_factory_ts_1.validator.custom('roomHotelConsistency', (value, req) => {
...
exports.bookingIdParamValidation = [
    validation_factory_ts_1.validator.integer('id', {
        required: true,
        min: 1,
    }),
];
...
exports.paginationQueryValidation = [
    validation_factory_ts_1.validator.integer('page', {
        required: false,
        min: 1,
    }),
    validation_factory_ts_1.validator.integer('limit', {
        required: false,
        min: 1,
...
exports.dateRangeFilterValidation = [
    validation_factory_ts_1.validator.date('startDate', {
        required: false,
        maxDate: new Date(), // Can't be in the future
    }),
    validation_factory_ts_1.validator.date('endDate', {
        required: false,
